/*
 * Workspaces-To-Dock extension for Gnome 3
 * Based on Dash-To-Panel extension
...
/** When true, ignore conflicting/GNOME-related settings and use safe defaults (matches prefs DISABLE_CONFLICTING_AND_GNOME_PREFS). */
export const USE_SAFE_DEFAULTS_CONFLICTING = true
...
export default class DashToWorkspacesExtension extends Extension {
  ...
  async enable() {
    DTP_EXTENSION = this
    SETTINGS = this.getSettings('org.gnome.shell.extensions.dash-to-workspaces')
    ...
    global.workspacesToDock = new EventEmitter()
    ...
    if (this.metadata.version != SETTINGS.get_int('extension-version')) {
      Utils.notify(
        _('Dash to Workspaces has been updated!'),
  ...
  enableGlobalStyles() {
    if (USE_SAFE_DEFAULTS_CONFLICTING) {
      this._dtwAddedBrClass = false
      return
    }
    let globalBorderRadius = SETTINGS.get_int('global-border-radius')
    let dtpActive = !!(global.dashToPanel && global.dashToPanel.panels)

    if (globalBorderRadius && !dtpActive) {
      Main.layoutManager.uiGroup.add_style_class_name(
        `br${globalBorderRadius * 4}`,
      )
      this._dtwAddedBrClass = true
    } else this._dtwAddedBrClass = false
  }

  disableGlobalStyles() {
    if (!this._dtwAddedBrClass) return
    if (global.dashToPanel?.panels) return
