...
const Preferences = class {
  constructor(window, settings, path) {
    // this._settings = ExtensionUtils.getSettings('org.gnome.shell.extensions.dash-to-workspaces');
    ...
    PanelSettings.setMonitorsInfo(settings).then(() => {
      this._bindSettings()
      if (DISABLE_CONFLICTING_AND_GNOME_PREFS) this._disableConflictingPrefs()
  ...
  _displayPanelPositionsForMonitor(monitorIndex) {
    ...
    let updateElementsSettings = () => {
      // 只允许修改以下元素的设置：显示应用程序按钮、活动按钮、桌面按钮、任务栏
      const allowedElements = [
        Pos.SHOW_APPS_BTN, // 显示应用程序按钮
        Pos.ACTIVITIES_BTN, // 活动按钮
        Pos.DESKTOP_BTN, // 桌面按钮
        Pos.TASKBAR, // 任务栏
      ]
      ...
      while (child != null) {
        if (allowedElements.includes(child.id)) {
      ...
      // 保留其他元素的现有设置（从原始配置中读取）
      panelElementPositions.forEach((el) => {
        if (!allowedElements.includes(el.element)) {
          newPanelElementPositions.push(el)
        }
      })
    ...
    // 只显示以下元素的设置：显示应用程序按钮、活动按钮、桌面按钮、任务栏
    const allowedElements = [
      Pos.SHOW_APPS_BTN, // 显示应用程序按钮
      Pos.ACTIVITIES_BTN, // 活动按钮
      Pos.DESKTOP_BTN, // 桌面按钮
    ...
    const filteredPositions = panelElementPositions.filter((el) =>
      allowedElements.includes(el.element),
    )
    ...
    filteredPositions.forEach((el) => {
      ...
      downBtn.connect('clicked', () =>
        upDownClickHandler(filteredPositions.length - 1),
      )
...
export default class DashToWorkspacesPreferences extends ExtensionPreferences {
